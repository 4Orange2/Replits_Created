...
from random import *
root = Tk()
s = Canvas(root, width=1000, height=700, background = "black")
s.pack()
...
for i in range (numstars):
    starsx.append(randint(0,1000))
    starsy.append(randint(0,700))
...
#Creating the earth using a for loop and trig
  ...
  # note: you have to use trig to make a spiral, that's just how it works
...
#Creating the laser right above wherever the earth is
    for l in range(n): #n = int(yC/20) which is the y value of earth divided by the speed of the laser per frame
        ...
        sleep(0.05)
        s.delete(laser)
    # this is the laser animation
        
#Deleting the earth after laser finishes its loop
    for e in range( 0, numdebris ): # earth is a list of rectangles and it deletes these rectangles very quickly in a for loop
        s.delete( earth[e] )
